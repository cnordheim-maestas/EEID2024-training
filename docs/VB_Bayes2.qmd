...
MCMC is the most commonly used numerical algorithm for generating posterior samples. A `r myblue("Markov Chain")` is a sequence of randomly generated numbers where each draw depends on the one immediately preceding it.
...
We often use `r myblue("Information Criterion")` to compare between models. You may be familiar with AIC. But we want to use something different when we're doing sample-based Bayesian comparisons. 

The two most common are `r myblue(" Deviance Information Criterion")` (DIC) and `r myred("widely-Applicable Information Criterion")` (wAIC). Both seeks to judge a model on how well it fits (based on the likelihood), penalized by the complexity of the model.
...
## Bayesian analysis using nimble/JAGS 
...
## `bayesTPC` philosophy for fitting TPCs
